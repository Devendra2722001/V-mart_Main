...
const ResetPassword = () => {
  ...
  const [data, setData] = useState({
    email:"", otp:"", password: "", conPassword:"",
  });
  ...
  const resetPassword = async (e) => {
    ...
    const res = await fetch("https://vmart-api.herokuapp.com/forgotpass", {
      ...
      body: JSON.stringify({
        email:email, code:otp, password:password, con_password:conPassword,
      })
    });
    if(res.status === 200){
    ...
    <div class="row d-flex justify-content-center align-items-center h-100">
      ...
      <div class="col-md-8 col-lg-6 col-xl-4">
        <div>
            ...
            <input type="text" class="form-control" placeholder="enter email id" name="email" value={data.email} onChange={handleInputs} />
            {error.email && <span className='text-danger font-weight-bold'>{error.email}</span>}
        </div>
        <div>
            <label class="labels">OTP</label>
            <input type="text" class="form-control" placeholder="enter otp" name="otp" value={data.otp} onChange={handleInputs} />
            {error.otp && <span className='text-danger font-weight-bold'>{error.otp}</span>}
        </div>
        <div>
            <label class="labels">New Password</label>
            <input type="password" class="form-control" placeholder="enter new password" name="password" value={data.password}  onChange={handleInputs} />
